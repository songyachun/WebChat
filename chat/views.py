...
def send_friend_list(request, user_id):
    ...
    if len(friends_query):
        for user in friends_query:
            ...
            # friends["birthday"] = user.userinfo.birthday # TODO 如果用户没有info表数据则会保存，需要修改
            friends["profile_head"] = str(user.userinfo.profile_head)
            print('------------------',friends["profile_head"] )
    ...
    if request.is_websocket():
        ...
        for messages in request.websocket:
            ...
            if not dataType_query:
                request.websocket.send(
                    b'{"code":102,"error":"The dataType is not existed"}')
                continue
            ...
            if step == 0:
                ...
                if not Messages.objects.filter(M_FromUserID=sender_query[0],
                    ...
                    request.websocket.send(
                        b'{"code":103,"error":"Donot send friend_request double"}')
                step = 1
            # 发送申请响应
            if step == 1:
            ...
            if step == 3:
                ...
                # 客户端在线时发送
                print("===step3.1===在线？",
                      client_list[reciver_query[0].username].is_closed())
...
def get_city(ip):
    ...
    city = re.findall(
        r'span class="Whwtdhalf w50-0">.*?省(\w+?)市.*?</span>', context, re.S)
    print(city)
    return city[0]
